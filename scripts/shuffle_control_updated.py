...
class InterpTrigramLM:
    ...
    def p2(self, u: str, w: str) -> float:
        ...
        den = sum(v for (uu, _), v in self.bi.items() if uu == u) + self.alpha * self.V
        return num / den if den > 0 else self.p1(w)

    def p3(self, v: str, u: str, w: str) -> float:
        num = self.tri.get((v, u, w), 0) + self.alpha
        den = sum(val for (vv, uu, _), val in self.tri.items() if vv == v and uu == u) + self.alpha * self.V
...
def plot_fig2(df_long: pd.DataFrame, out_png: Path, seed: int):
    ...
    For each label (ADVICE/STORY), show ΔH distributions under two base texts:
      - base = original text (condition = "orig")
      - base = sentence-permuted text (condition = "sent_shuf")
    ΔH is always computed as H(token-shuffled) − H(base).
    """

    # ---- color palette close to your current Fig2 ----
    # ADVICE: blue (orig), orange (sent_shuf)
    # STORY : green (orig), red (sent_shuf)
    COLORS = {
    ...
    # ---- layout: a bit wider + more bottom space for labels ----
    fig, ax = plt.subplots(figsize=(8.2, 4.6))

    # positions: for each label i, orig at i-0.22, sent_shuf at i+0.22
    positions = []
    boxdata = []
    facecolors = []
    xtick_labels = []
    ...
    for i, lab in enumerate(labels, start=1):
        for cond, dx in zip(conds, [-0.22, +0.22]):
            ...
            n = len(arr)
            # short tick label: condition + n
            tick = "orig" if cond == "orig" else "sent-shuf"
            xtick_labels.append(f"{tick}\n(n={n})")

    # Boxplot (patch_artist to color boxes)
    bp = ax.boxplot(
        boxdata,
        positions=positions,
        widths=0.33,
    ...
    # Jitter points (same color as the corresponding box)
    ...
    for (lab, cond), ys in data.items():
        ...
        ax.scatter(
            ...
            alpha=0.45,
            linewidths=0,
            color=COLORS[(lab, cond)]
        )

    # Axis formatting
    ax.set_xlim(0.4, 2.6)
    ax.set_xticks(positions)
    ax.set_xticklabels(xtick_labels, linespacing=1.15)
    ax.set_ylabel(r"$\Delta H$ (bits/token)")

    # Group labels under the two pairs (ADVICE / STORY)
    # Put them slightly below tick labels, without crowding the plot area.
    ax.text(1.0, -0.22, "ADVICE", transform=ax.get_xaxis_transform(),
            ha="center", va="top", fontsize=13)
    ax.text(2.0, -0.22, "STORY", transform=ax.get_xaxis_transform(),
            ha="center", va="top", fontsize=13)

    # A subtle separator between groups
    ax.axvline(1.5, color="black", linewidth=0.8, alpha=0.15)

    # Light y-grid for readability (won't look crowded)
    ax.yaxis.grid(True, alpha=0.18)
    ax.set_axisbelow(True)

    # Move the explanatory note outside the axes (cleaner than in-plot text)
    fig.text(
        0.12, 0.02,
        "Within each label: left = base (original), right = base (sentence-permuted).",
        ha="left", va="bottom", fontsize=10
    )

    # Tight layout with extra bottom margin
